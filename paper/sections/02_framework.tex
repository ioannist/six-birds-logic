...
h = \tilde h \circ f.
\]
We write \(\Sigma_f\) for the family of events or predicates definable from \(f\). \(\Sigma_f\) is the expressive content of the layer: if a distinction is not definable there, then it is not available as a logical statement at that layer \cite{pawlak1982rough,ellerman2010partitions,shalizi2025macrostate}.
...
In finite-state language, the same descent problem is the one studied by lumpability theory for Markov chains \cite{kemeny1976finite,buchholz1994lumpability,simonando1961aggregation,givon2004extracting}.
...
\begin{figure}[t]
    \centering
    \resizebox{0.94\linewidth}{!}{\input{figures/logic_layer_overview.tex}}
